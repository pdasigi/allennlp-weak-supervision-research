...
class ExpectedRiskMinimization(DecoderTrainer[Callable[[StateType], torch.Tensor]]):
    ...
    performing beam search or sampling. See "Classical Structured Prediction Losses for Sequence to Sequence
    Learning" by Edunov et al., 2017 for more details (they use beam search alone, but we also
    implement a sampling variant of their algorithm).
